...
## 导入
[图文教程](https://delta-kimigatame.hatenablog.jp/entry/ar1716787)
...
#### `void Input()`
使用 shift_Jis 打开`UtauPlugin.filePath`文件，并解析内容。

Ust头信息保存在`UtauPlugin UtauPlugin`中，音符信息保存在`List UtauPlugin.note`中。

#### `void Output()`
将编辑的内容覆盖到`UtauPlugin.filePath`文件中。

符合UTAU插件的规范，并使输出的内容尽可能少。

|要素|是否输出|
|:-------|:-------|
|标题信息|否|
|[PREV][NEXT]|否|
|[#DELETE]|仅输出[#DELETE]行，不输出内容|
|[#INSERT]|完整输出|
|其他|仅输出变化的内容|
